...
import io.github.ramerf.blog.system.entity.request.common.MenuRequest;
...
public class MenuValidator implements Validator {
  ...
  public void validate(final Object target, @Nonnull final Errors errors) {
    ...
    if (menu == null) {
      ...
      if (StringUtils.isEmpty(menu.getName()) || menu.getUrl().length() > 50) {
        errors.rejectValue("name", "menu.name.length", "名称 不能为空且小于50个字符");
      }
      if (StringUtils.isEmpty(menu.getUrl()) || menu.getUrl().length() > 50) {
        errors.rejectValue("url", "menu.url.length", "地址 不能为空且小于50个字符");
      }
