...
(function($) {
  ...
  $.fn.resetAutosize = function(options) {
    ...
    const minWidth = $(this).data('minwidth') || options.minInputWidth ||
        $(this).width(),
  ...
  $.fn.removeTag = function(value) {
    ...
    this.each(function() {
      ...
      if (tags_callbacks[id] && tags_callbacks[id]['onRemoveTag']) {
        const f = tags_callbacks[id]['onRemoveTag'];
        f.call(this, value);
      }
    });
  ...
  $.fn.tagExist = function(val) {
    const id = $(this).attr('id');
    const tagslist = $(this).val().split(delimiter[id]);
      ...
      if (settings.interactive) {
        ...
        if (settings.autocomplete_url != undefined) {
          ...
          if (jQuery.Autocompleter !== undefined) {
            $(data.fake_input).
                autocomplete(settings.autocomplete_url, settings.autocomplete);
            $(data.fake_input).
            ...
            $(data.fake_input).
                bind('autocompleteselect', data, function(event, ui) {
            ...
            if ($(event.data.fake_input).val() != '' &&
              ...
              if ((event.data.minChars <=
                  ...
                  (!event.data.maxChars || (event.data.maxChars >=
                      $(event.data.fake_input).val().length)))
                $(event.data.real_input).
                    addTag($(event.data.fake_input).val(),
                        {focus: true, unique: (settings.unique)});
            } else {
              $(event.data.fake_input).
                  val($(event.data.fake_input).attr('data-default'));
  ...
  $.fn.tagsInput.importTags = function(obj, val) {
    ...
    if (tags_callbacks[id] && tags_callbacks[id]['onChange']) {
      const f = tags_callbacks[id]['onChange'];
