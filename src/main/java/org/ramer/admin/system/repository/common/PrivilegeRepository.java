...
public interface PrivilegeRepository extends BaseRepository<Privilege, Long> {
  ...
  @Query(
      "select p from org.ramer.admin.system.entity.domain.common.Role r inner join r.privileges p where r.id= :roleId and p.state= :state")
  List<Privilege> findByRoles(@Param("roleId") Long roleId, @Param("state") int state);
}
