...
($ => {
  ...
  class BootstrapTable {
    ...
    initContainer() {
      const topPagination = ['top', 'both'].includes(
          this.options.paginationVAlign)
          ? '<div class="fixed-table-pagination clearfix"></div>' : '';
      const bottomPagination = ['bottom', 'both'].includes(
          this.options.paginationVAlign)
    ...
    initHeader() {
      ...
      this.options.columns.forEach((columns, i) => {
        ...
        columns.forEach((column, j) => {
          ...
          halign = Utils.sprintf('text-align: %s; ',
          ...
          html.push(`<th${Utils.sprintf(' title="%s"', column.titleTooltip)}`,
              column.checkbox || column.radio
                  ? Utils.sprintf(' class="bs-checkbox %s"',
                  column['class'] || '')
                  : class_,
          ...
          html.push(Utils.sprintf('<div class="th-inner %s">',
              this.options.sortable && column.sortable
                  ? 'sortable both' : ''));
      ...
      if (index !== -1) {
        ...
        if (this.options.customSort) {
          ...
          this.data.sort((a, b) => {
            ...
            const value = Utils.calculateObjectValue(this.header,
                this.header.sorters[index], [aa, bb, a, b]);
        ...
        if (this.options.sortClass !== undefined) {
          ...
          timeoutId = setTimeout(() => {
            ...
            const index = this.$header.find(
                `[data-field="${this.options.sortName}"]`).index();
            this.$el.find(`tr td:nth-child(${index + 1})`).
    ...
    onSort({type, currentTarget}) {
      const $this = type === 'keypress' ? $(currentTarget) : $(currentTarget).
      parent();
      ...
      if (this.options.sortName === $this.data('field')) {
        ...
        if (this.options.rememberOrder) {
          ...
          this.options.sortOrder = this.columns[this.fieldsColumnsIndex[$this.data(
              'field')]].order;
        }
      }
    ...
    initToolbar() {
      ...
      if (typeof o.toolbar === 'string' || typeof o.toolbar === 'object') {
        $(Utils.sprintf('<div class="bs-bars %s-%s"></div>',
            this.constants.classes.pull, o.toolbarAlign)).
      ...
      if (o.showPaginationSwitch) {
        html.push(`<button class="${this.constants.buttonsClass}" type="button" name="paginationSwitch"
          ...
          ${Utils.sprintf(this.constants.html.icon, o.iconsPrefix,
      ...
      if (o.showColumns) {
        html.push(
            `<div class="keep-open ${this.constants.classes.buttonsDropdown}" title="${o.formatColumns()}">
      ...
      if (o.showPaginationSwitch) {
        this.$toolbar.find('button[name="paginationSwitch"]').
             off('click').
             on('click', () => this.togglePagination());
      }
      ...
      if (o.showColumns) {
        ...
        $keepOpen.find('input').off('click').on('click', ({currentTarget}) => {
          ...
          this.trigger('column-switch', $this.data('field'),
              $this.prop('checked'));
        });
      }
      ...
      if (o.search) {
        ...
        html.push(
          ...
          <input class="${this.constants.classes.input}${Utils.sprintf(
                ' input-%s', o.iconSize)}"
    ...
    initSearch() {
      if (this.options.sidePagination !== 'server') {
        if (this.options.customSearch) {
          this.data = Utils.calculateObjectValue(this.options,
        ...
        const f = Utils.isEmptyObject(this.filterColumns)
            ? null
            : this.filterColumns;
        ...
        this.data = s ? this.data.filter((item, i) => {
          for (let j = 0; j < this.header.fields.length; j++) {
            ...
            const key = Utils.isNumeric(this.header.fields[j]) ? parseInt(
                this.header.fields[j], 10) : this.header.fields[j];
    ...
    initPagination() {
      ...
      html.push(
          `<div class="${this.constants.classes.pull}-${o.paginationDetailHAlign} pagination-detail">
      ...
      if (!o.onlyInfoPagination) {
        ...
        pageList.forEach((page, i) => {
          if (!o.smartDisplay || i === 0 || pageList[i - 1] < o.totalRows) {
            let active;
            if ($allSelected) {
              active = page === o.formatAllRows()
                  ? this.constants.classes.dropdownActive
                  : '';
            } else {
              active = page === o.pageSize
                  ? this.constants.classes.dropdownActive
                  : '';
            }
            pageNumber.push(
                Utils.sprintf(this.constants.html.pageDropdownItem, active,
                    page));
          }
        });
        ...
        html.push(
            `<div class="${this.constants.classes.pull}-${o.paginationHAlign} pagination">`,
            Utils.sprintf(this.constants.html.pagination[0],
                Utils.sprintf(' pagination-%s', o.iconSize)),
            Utils.sprintf(this.constants.html.paginationItem, ' page-pre',
                o.paginationPreText));
        ...
        if (o.paginationSuccessivelySize > this.totalPages - from) {
          from = from -
                 (o.paginationSuccessivelySize - (this.totalPages - from)) + 1;
        }
        ...
        const pageItem = (i, classes = '') => {
          return Utils.sprintf(this.constants.html.paginationItem,
              classes + (i === o.pageNumber
              ? ` ${this.constants.classes.paginationActive}`
              : ''), i);
        };
        ...
        if (from > 1) {
          ...
          if ((from - 1) === max + 1) {
            ...
            if ((from - 1) > max) {
              if (
                  (from - o.paginationPagesBySide * 2) >
                  o.paginationPagesBySide &&
        ...
        html.push(
            Utils.sprintf(this.constants.html.paginationItem, ' page-next',
      ...
      if (!o.onlyInfoPagination) {
        ...
        $number = this.$pagination.find('.page-item').
    ...
    onPageListChange(event) {
      ...
      this.options.pageSize = $this.text().toUpperCase() ===
    ...
    initRow(item, i, data, parentDom) {
      ...
      if (!this.options.cardView && this.options.detailView) {
        ...
        if (Utils.calculateObjectValue(null, this.options.detailFilter,
            [i, item])) {
          html.push(`
            <a class="detail-icon" href="#">
            ${Utils.sprintf(this.constants.html.icon, this.options.iconsPrefix,
      ...
      this.header.fields.forEach((field, j) => {
        ...
        if (csses.concat([this.header.styles[j]]).length) {
          style_ = ` style="${csses.concat([this.header.styles[j]]).
                                    join('; ')}"`;
        }
        ...
        if (item[`_${field}_data`] &&
            !Utils.isEmptyObject(item[`_${field}_data`])) {
        ...
        if (column.checkbox || column.radio) {
          ...
          const isDisabled = !column.checkboxEnabled ||
          ...
          text = [
            ...
            `<label>
              ...
              ${Utils.sprintf('checked="%s"',
                isChecked ? 'checked' : undefined)}
              ${Utils.sprintf('disabled="%s"',
          ...
          item[this.header.stateField] = value === true ||
                                         (!!value_ || (value && value.checked));
        } else {
          ...
          if (this.options.cardView) {
            const cardTitle = this.options.showHeader
                ? `<span class="card-view-title"${style_}>${Utils.getFieldTitle(
                    this.columns, field)}</span>` : '';
    ...
    initBody(fixedScroll) {
      ...
      // show no records
      if (!hasTr) {
        this.$body.html(`<tr class="no-records-found">${Utils.sprintf(
            '<td colspan="%s">%s</td>',
      ...
      this.$body.find('> tr[data-index] > td').
           off('click dblclick').
           on('click dblclick', ({currentTarget, type, target}) => {
             ...
             const index = this.options.cardView ? $cardviewArr.index(
                 $cardviewTarget) : $td[0].cellIndex;
             const fields = this.getVisibleFields();
             const field = fields[this.options.detailView &&
                                  !this.options.cardView ? index - 1 : index];
             ...
             this.trigger(type === 'click' ? 'click-cell' : 'dbl-click-cell',
                 field, value, item, $td);
             ...
             if (
                 ...
                 !Utils.calculateObjectValue(this.options,
                     this.options.ignoreClickToSelectOn, [target])
             ) {
               const $selectItem = $tr.find(
      ...
      this.$body.find('> tr[data-index] > td > .detail-icon').
           off('click').
           on('click', e => {
             e.preventDefault();
             ...
             if ($tr.next().is('tr.detail-view')) {
               $this.html(Utils.sprintf(this.constants.html.icon,
               ...
               const content = Utils.calculateObjectValue(this.options,
                   this.options.detailFormatter, [index, row, $element], '');
               if ($element.length === 1) {
      ...
      this.header.events.forEach((_events, i) => {
        ...
        for (const [key, event] of Object.entries(events)) {
          this.$body.find('>tr:not(.no-records-found)').each((i, tr) => {
            const $tr = $(tr);
            const $td = $tr.find(this.options.cardView ? '.card-view' : 'td').
                            eq(fieldIndex);
    ...
    updateSelected() {
      const checkAll = this.$selectItem.filter(':enabled').length &&
                       ...
                       this.$selectItem.filter(':enabled').
                            filter(':checked').length;

      this.$selectAll.add(this.$selectAll_).prop('checked', checkAll);

      this.$selectItem.each((i, el) => {
        $(el).closest('tr')[$(el).prop('checked') ? 'addClass' : 'removeClass'](
            'selected');
      });
    }
    ...
    updateRows() {
      this.$selectItem.each((i, el) => {
        this.data[$(el).data('index')][this.header.stateField] = $(el).
        prop('checked');
      });
    }
    ...
    resetHeader() {
      ...
      this.timeoutId_ = setTimeout(() => this.fitHeader(),
    ...
    fitHeader() {
      ...
      this.$header.find('th[data-field]').each((i, el) => {
        this.$header_.find(
            Utils.sprintf('th[data-field="%s"]', $(el).data('field'))).
             data($(el).data());
      });
      ...
      $tr.find('> *').each((i, el) => {
        ...
        if (this.options.detailView && !this.options.cardView) {
          if (i === 0) {
            ...
            const zoomWidth = $thDetail.width() -
    ...
    resetFooter() {
      ...
      for (const column of this.columns) {
        ...
        falign = Utils.sprintf('text-align: %s; ',
        ...
        html.push('<th', class_, Utils.sprintf(' style="%s"',
            falign + valign + csses.concat().join('; ')), '>');
        html.push('<div class="th-inner">');

        html.push(
            Utils.calculateObjectValue(column, column.footerFormatter, [data],
                ''));
    ...
    toggleColumn(index, checked, needUpdate) {
      ...
      if (this.options.showColumns) {
        const $items = this.$toolbar.find('.keep-open input').
                            prop('disabled', false);

        if (needUpdate) {
          $items.filter(Utils.sprintf('[value="%s"]', index)).
    ...
    resetView(params) {
      ...
      if (this.options.height) {
        ...
        this.$tableBorder && this.$tableBorder.css('height',
            `${height - tableHeight - padding - 1}px`);
      }
    ...
    updateByUniqueId(params) {
      ...
      for (const params of allParams) {
        ...
        const rowId = this.options.data.indexOf(
            this.getRowByUniqueId(params.id));
    ...
    refreshColumnTitle(params) {
      ...
      if (this.columns[this.fieldsColumnsIndex[params.field]].visible) {
        const header = this.options.height !== undefined
            ? this.$tableHeader
            : this.$header;
        ...
        if (executeInterval) {
          try {
            idInterval = setInterval(() => {
              detailIcon = this.$body.find('tr.detail-view').
                                last().
                                find('.detail-icon');
  ...
  $.fn.bootstrapTable = function(option, ...args) {
    ...
    this.each((i, el) => {
      ...
      if (!data) {
        $(el).
        data('bootstrap.table', (data = new $.BootstrapTable(el, options)));
      }
    });
